...
def test_blank_channel():
    ...
    assert blank_chan.__repr__() == '<Channel: sources=CSRC.no_source, id=None, xmltv_id=None, number=None, name=None>'
...
def test_channel_from_skyq_service():
    ...
    assert chan.__repr__() == '<Channel: sources=CSRC.skyq_service_summary, id=2002, xmltv_id=None, number=101, name=BBC One Lon>'
    assert chan.c == "101"
    ...
    assert chan.__repr__() == '<Channel: sources=CSRC.skyq_service_detail|skyq_service_summary, id=2002, xmltv_id=None, number=101, name=BBC One Lon>'
...
def test_channel_from_xmltv_data():
    ...
    assert isinstance(chan, Channel)
    assert chan.sources == CSRC.xml_tv
    assert chan.__repr__() == '<Channel: sources=CSRC.xml_tv, id=None, xmltv_id=015ac32387fbc90e1e920d780d43787c, number=None, name=None>'
...
def test_channel_from_both_sources():
    ...
    assert isinstance(chan, Channel)
    assert chan.sources == CSRC.xml_tv | CSRC.skyq_service_summary
    assert chan.__repr__() == '<Channel: sources=CSRC.xml_tv|skyq_service_summary, id=2002, xmltv_id=015ac32387fbc90e1e920d780d43787c, number=101, name=BBC One Lon>'
    assert chan.c == "101"
    assert chan.t == "BBC One Lon"
    assert chan.name == "BBC One Lon"
    assert chan.xmltv_id == "015ac32387fbc90e1e920d780d43787c"
